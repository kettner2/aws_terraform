...
module "vpc" {
  ...
  # bastion, LB 용도 (NAT GW도 여기 배치) — /27 (32 IPs)
  public_subnets = [cidrsubnet(var.vpc_primary_cidr, 3, 0)]

  # EKS 노드 전용 — /26 (64 IPs)
  private_subnets = [cidrsubnet(var.vpc_primary_cidr, 2, 1)]
...
# Pod 전용 서브넷 (보조 CIDR) — /22 (1024 IPs)
